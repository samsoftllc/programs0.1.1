...
- External assets are DISABLED (files = off). Everything renders with rectangles, 
  circles, and text. Pure code graphics only.
...
- Names in comments are purely for reference to gameplay styles. No third‑party
  assets are included. All graphics are procedurally generated.
...
class Player:
    ...
    def draw(self, surface, camera_x):
        ...
        # eyes
        eye_x = r.centerx + (4 * self.facing)
        pygame.draw.circle(surface, BLACK, (eye_x, r.y + 10), 3)
...
class Walker:
    ...
    def __init__(self, x, y, dir=1):
        self.rect = pygame.Rect(x, y, 22, 18)
        self.vx = 1.2 * dir
        self.alive = True
        self.squash_timer = 0
    ...
    def draw(self, surface, camera_x):
        ...
        if not self.alive:
            ...
            r.height = 8
            r.y += 10
            pygame.draw.rect(surface, body, r, border_radius=3)
        else:
            pygame.draw.rect(surface, body, r, border_radius=6)
            pygame.draw.circle(surface, BLACK, (r.centerx-4, r.y+6), 2)
            pygame.draw.circle(surface, BLACK, (r.centerx+4, r.y+6), 2)
...
class Game:
    ...
    def loop_title(self):
        ...
        # Draw parting curtains (procedural)
