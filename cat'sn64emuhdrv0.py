...
class Emulator:
    def __init__(self, rom_path: str):
        ...
        self.memory = Memory(rom, pif_rom)
        self.cpu = CPU()
        # Init PIF bootstrap
        self.bootstrap_pif()
...
class PJ64LegacyGUI:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Project64 1.6 Legacy - CatSeekR1")
        self.root.geometry("600x400")
        self.root.resizable(False, False)
        
        # PJ64 1.6 style colors
        self.bg_color = "#C0C0C0"  # Classic gray
        self.menu_bg = "#E0E0E0"
    ...
    def create_menu(self):
        ...
        # File menu
        file_menu = tk.Menu(menubar, tearoff=0, bg=self.menu_bg)
        ...
        # System menu
        system_menu = tk.Menu(menubar, tearoff=0, bg=self.menu_bg)
        ...
        # Options menu
        options_menu = tk.Menu(menubar, tearoff=0, bg=self.menu_bg)
        options_menu.add_command(label="Settings", command=self.show_settings)
        menubar.add_cascade(label="Options", menu=options_menu)
        
        # Help menu
        help_menu = tk.Menu(menubar, tearoff=0, bg=self.menu_bg)
    ...
    def create_toolbar(self):
        ...
        buttons = [
            ("Open", self.open_rom),
            ("Play", self.start_emulator),
            ("Pause", self.pause_emulator),
            ("Stop", self.stop_emulator),
            ("Reset", self.reset_emulator)
        ]
        
        for text, command in buttons:
            btn = tk.Button(toolbar, text=text, command=command, 
                          bg=self.menu_bg, relief=tk.RAISED, bd=1)
    ...
    def open_rom(self):
        ...
        if filename:
            try:
                self.emulator = Emulator(filename)
    ...
    def show_settings(self):
        ...
        tk.Label(settings_window, text="PJ64 1.6 Legacy Settings", 
                bg=self.bg_color, font=("Arial", 12, "bold")).pack(pady=10)
        
        # Files = Off setting (mimicking PJ64 1.6)
        files_var = tk.BooleanVar(value=True)  # Default off
        tk.Checkbutton(settings_window, text="Files = Off", 
                      variable=files_var, bg=self.bg_color).pack(pady=5)
        
        tk.Button(settings_window, text="Apply", 
                 command=settings_window.destroy).pack(pady=10)
        
    def show_about(self):
        about_text = """Project64 1.6 Legacy - CatSeekR1 Edition

N64 Emulator based on PJ64 1.6 design
600x400 window size
Files = Off mode enabled

[C] SamSoft LLC 1999-2025
[C] CatSeekR1 Enhanced"""
